 ...
 init() {
        ...
        const handlePopState = (event) => {
            ...
            if (event.state && event.state.fileId) {
                ...
                if (file) {
                    this.currentFile = { ...file, folder: null, folderPath: [] };
                    ...
                    console.log('✅ Restored file via popstate (ke file view):', fileId);
                } else {
            ...
            if (event.state && event.state.folderId) {
                ...
                const folder = this.folders.find(f => String(f.id) === String(event.state.folderId));
                if (folder) {
                    this.currentFolder = folder;
                    this.currentFile = null;
                    this.updateBreadcrumbs();
                    this.loadMembersFromAPI();
                    console.log('✅ Restored folder via popstate:', folder.name);
                } else {
                    this.currentFolder = null;
                    this.currentFile = null;
                    this.updateBreadcrumbs();
  ...
  // helper central untuk mengatur history (push / replace)
    ...
    openFolder(folder) {
        ...
        this.$nextTick(() => {
            ...
            this.setHistoryState({ folderId: folder.id, folderName: folder.name }, false);
    ...
    restoreFolderFromUrl() {
        ...
        if (fileIdFromUrl) {
            ...
            const allFiles = this.getAllFiles(this.folders);
            const file = allFiles.find(f => String(f.id) === String(fileIdFromUrl));
            if (file) {
                this.currentFile = { ...file, folder: null, folderPath: [] };
                this.currentFileUploadedBy = file.uploaded_by || file.uploader?.id || null;
                this.currentFolder = null;
                ...
                this.setHistoryState({ fileId: file.id, folderId: folderIdFromUrl || null }, true);
                console.log('✅ Restored file from URL (replace state):', fileIdFromUrl);
            } else {
                this.currentFile = null;
                // no file found -> fallback to folder handling
                if (folderIdFromUrl) {
                    const folder = this.folders.find(f => String(f.id) === String(folderIdFromUrl));
                    if (folder) {
                        this.currentFolder = folder;
                        this.currentFile = null;
                        this.updateBreadcrumbs();
                        this.setHistoryState({ folderId: folder.id }, true);
                        console.log('✅ Fallback: restored folder from URL (replace state):', folder.id);
                    } else {
                        this.currentFolder = null;
        ...
        if (folderIdFromUrl) {
            ...
            if (folder) {
                ...
                console.log('✅ Folder restored dari URL ?folder=', folderIdFromUrl, folder.name);
            } else {
                this.currentFolder = null;
                this.setHistoryState({}, true);
            }
    ...
    openFile(file) {
        ...
        this.setHistoryState({ fileId: file.id, folderId: folderId }, false);
