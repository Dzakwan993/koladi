    ...
    <div class="max-w-7xl mx-auto bg-white rounded-xl border shadow-sm">
        ...
        <div x-show="!loadingTimeline && timelineData.length > 0"
            ...
            <div class="flex justify-between text-xs text-gray-600 mt-2">
                <span
                    x-text="`${timelineData.reduce((s, p) => s + p.completed_tasks, 0)} selesai`"></span>
                <span
                    x-text="`${timelineData.reduce((s, p) => s + p.total_tasks, 0)} total tugas`"></span>
            </div>
        </div>
        ...
        <template x-for="phase in getProjectPhases()" :key="phase.id">
            <div class="flex border-b hover:bg-gray-50 transition">
                ...
                <!-- Phase Info -->
                <div class="w-72 px-4 py-4 border-r bg-white cursor-pointer"
                     @click="showPhaseTasks(phase.id)">

                    <div class="flex items-center justify-between mb-2">
                        <h3 class="text-sm font-semibold text-gray-800"
                            x-text="phase.name"></h3>
                        ...
                        <span class="text-xs px-2 py-1 rounded-md font-medium"
                              ...
                              x-text="`${phase.progress_percentage}%`"></span>
                    </div>
                ...
                <div class="flex-1 relative bg-white h-20">
                    ...
                    <div class="absolute top-1/2 -translate-y-1/2 h-8 rounded-lg transition-all duration-500
                        ...
                        <div class="relative h-full flex items-center justify-center text-xs text-gray-700 font-medium">
                            <span x-text="`${phase.progress_percentage}%`"></span>
                        </div>
                    </div>
