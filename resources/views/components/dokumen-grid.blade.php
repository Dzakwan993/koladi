    ...
    <div
        ...
        <template x-for="document in getDisplayedDocuments()" :key="document.id">
            <div @click="selectMode ? toggleDocumentSelection(document) : (document.type === 'Folder' ? openFolder(document) : openFile(document))"
                ...
                <!-- Nama File/Folder (max 2 baris) -->
                <span 
                    class="text-xs font-medium text-gray-700 w-full break-words line-clamp-2"
                    x-text="document.name"
                    :title="document.name">
                </span>

                <!-- Tipe dan Secret Icon -->
                <div class="flex items-center gap-1 mt-1">
                    <span x-show="document.type" class="text-xs text-gray-400" x-text="document.type"></span>
