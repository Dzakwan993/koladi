...
<template x-if="filteredDocuments.length === 0 && searchQuery.length === 0 && !currentFolder && !currentFile">
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-x-6 gap-y-3 items-start">
        ...
        <template x-for="folder in getRootFolders()" :key="folder.id">
            <div @click="selectMode ? toggleDocumentSelection(folder) : openFolder(folder)" :class="{
                ...
                <div x-show="selectMode" class="absolute top-2 right-2">
                    <div :class="isDocumentSelected(folder.id) ? 'bg-blue-600 border-blue-600' : 'bg-white border-gray-300'"
                        ...
                        <svg x-show="isDocumentSelected(folder.id)" class="w-3 h-3 text-white" fill="none"
                ...
                <span class="text-sm font-medium text-gray-700 w-full break-words line-clamp-2" x-text="folder.name"
                    :title="folder.name">
                </span>
                ...
                <div class="flex items-center gap-1 mt-1">
                    ...
                    <template x-if="folder.isSecret">
                        <svg class="w-3 h-3 text-yellow-500 flex-shrink-0" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
