                                    ...
                                    <div class="flex items-start justify-between gap-2">
                                        <div class="flex-1">
                                            ...
                                            <div x-show="getChildCount(node.id) > 0" class="mt-2">
                                                <span
                                                    :class="node.isRoot || node.type === 'idea' || node.type === 'task' ? 'bg-white/20 text-white' : 'bg-gray-100 text-gray-700'"
                                        ...
                                        <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <button @click.stop="addChildNode(node)"
                                                :class="node.isRoot || node.type === 'idea' || node.type === 'task' ? 'text-white hover:bg-white/20' : 'text-gray-600 hover:bg-gray-100'"
            ...
            function mindmapApp() {
                return {
                    ...
                    async init() {  // TAMBAH async
                        ...
                        this.$watch('nodes', () => { this.scheduleRedraw(); });
                        this.$watch('zoomLevel', () => { this.scheduleRedraw(); });
                        this.$watch('panX', () => { this.scheduleRedraw(); });
                        this.$watch('panY', () => { this.scheduleRedraw(); });
                    },
                    ...
                    async loadData() {
                        try {
                            ...
                            alert('Gagal memuat data mindmap');
                        }
                    },

                    // 2. Save node ke database
                    async saveNodeToDb(node, isNew = false) {
                        if (this.isSyncing) return;
                        this.isSyncing = true;

                        try {
                            const url = isNew ? '/api/mindmap/nodes' : `/api/mindmap/nodes/${node.id}`;
                            const response = await fetch(url, {
                                ...
                                body: JSON.stringify({
                                    ...
                                    connection_side: node.connectionSide,
                                }),
                            });
                    ...
                    // 3. Update posisi (drag)
                    async updatePosition(node) {
                        if (this.isSyncing) return;
                        this.isSyncing = true;

                        try {
                            await fetch(`/api/mindmap/nodes/${node.id}/position`, {
                                method: 'PUT',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                    ...
                    addChildNode(parent) {
                        ...
                        this.saveNodeToDb(newNode, true);
                    ...
                    saveNode() {
                        ...
                        if (this.editingNode.id) {
                            ...
                            if (index !== -1) {
                                ...
                                this.saveNodeToDb(this.nodes[index], false);
                            }
                            this.pushHistory('editNode');
                        } else {
                            this.editingNode.id = Date.now();
                            this.nodes.push({ ...this.editingNode });
                            this.saveNodeToDb(this.editingNode, true);
                            this.pushHistory('addNodeModal');
                        }

                        this.showModal = false;  // PERBAIKI TYPO
                        this.editingNode = {};
                    },
                    ...
                    createConnection(source, target, sourceSide) {
                        ...
                        this.saveNodeToDb(target, false);
                    },
                    ...
                    stopDrag() {
                        if (this.draggingNode) {
                            this.updatePosition(this.draggingNode);
