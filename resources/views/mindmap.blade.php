...
@section('content')
                ...
                <div
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div class="flex items-center gap-2">
                            <button @click="undo()"
                                :class="undoStack.length ? 'bg-white text-gray-700' :
                                    'bg-gray-50 text-gray-300 cursor-not-allowed'"
                                class="px-3 py-2 rounded-lg border border-gray-200 text-sm font-medium"
                                :disabled="undoStack.length === 0" title="Undo (Ctrl+Z)">
                                ↶ Undo
                            </button>

                            <button @click="redo()"
                                :class="redoStack.length ? 'bg-white text-gray-700' :
                                    'bg-gray-50 text-gray-300 cursor-not-allowed'"
                                class="px-3 py-2 rounded-lg border border-gray-200 text-sm font-medium"
                    ...
                    <div class="flex items-center gap-4 text-sm">
                        ...
                        <div class="flex items-center gap-2 bg-green-50 px-4 py-2 rounded-lg border border-green-200">
                            ...
                            <span class="font-medium text-green-700"><span x-text="connectionCount">0</span>
                                Connections</span>
                        </div>
                    </div>
                ...
                <div id="mindmap-area" class="relative"
                    ...
                    <template x-for="node in nodes" :key="node.id">
                        <div :id="`node-${node.id}`" :data-node-id="node.id"
                            ...
                            @mousedown.stop="startDrag(node, $event)" @dblclick="editNode(node)"
                                    ...
                                    <div class="flex items-start justify-between gap-2">
                                        <div class="flex-1">
                                            ...
                                            <div x-show="getChildCount(node.id) > 0" class="mt-2">
                                                <span
                                                    :class="node.isRoot || node.type === 'idea' || node
                                                        .type === 'task' ? 'bg-white/20 text-white' :
                    ...
                    <div class="p-6 space-y-4">
                        <div>
                            ...
                            <input x-model="editingNode.title" type="text"
                                ...
                                placeholder="Masukkan judul node">
                        </div>
                        ...
                        <div x-show="!editingNode.isRoot">
                            ...
                            <select x-model="editingNode.parentId"
                                ...
                                <template x-for="parent in nodes.filter(n => n.id !== editingNode.id)"
                                    :key="parent.id">
    ...
    @push('scripts')
        ...
        <script>
            // GANTI SELURUH function mindmapApp() YANG LAMA DENGAN YANG BARU INI:
            function mindmapApp(mindmapId) {
                return {
                    ...
                    animationFrame: null,
                    pendingDrag: null,
                    autoSaveTimeout: null, // BARU DITAMBAH
                    ...
                    init() {
                        ...
                        this.keyHandler = (e) => {
                            if ((e.ctrlKey || e.metaKey) && !e.shiftKey && e.key.toLowerCase() === 'z') {
                                e.preventDefault();
                                this.undo();
                            } else if ((e.ctrlKey || e.metaKey) && (e.key.toLowerCase() === 'y' || (e.shiftKey && e.key
                                    .toLowerCase() === 'z'))) {
                                e.preventDefault();
                                this.redo();
                            }
                        ...
                        // BARU: Watch untuk perubahan nodes dan auto-save
                        this.$watch('nodes', () => {
                    ...
                    async saveToDatabase() {
                        try {
                            ...
                            alert('Gagal menyimpan mindmap: ' + error.message);
                        }
                    },
                    ...
                    async loadMindmapData() {
                        try {
                            ...
                            // BARU: Jangan load data jika sedang edit mode
                            if (this.showModal || this.draggingNode || this.isConnecting) {
                                console.log('Skip load - sedang dalam mode edit');
                                return;
                            }
                            ...
                            // Fallback ke node default jika gagal load
                            this.createDefaultNode();
                        }
                    },

                    createDefaultNode() {
                        // Create initial root node jika load dari database gagal
                        this.nodes = [{
                    ...
                    initPusher() {
                        // Inisialisasi Pusher untuk real-time updates
                        if (typeof Pusher !== 'undefined') {
                            const pusher = new Pusher('{{ config('broadcasting.connections.pusher.key') }}', {
                                cluster: '{{ config('broadcasting.connections.pusher.options.cluster') }}'
                            });

                            const channel = pusher.subscribe(`mindmap-${this.mindmapId}`);
                            channel.bind('mindmap-updated', (data) => {
                                console.log('Mindmap updated via Pusher:', data);

                                // BARU: Jangan langsung reload, tapi sync dengan hati-hati
                    ...
                    drawTempConnection() {
                        ...
                        const startPoint = this.getConnectionPoint(this.connectionSource, this.connectionSourceSide, startX,
                            startY, fromRect);
                        ...
                        if (this.connectionStyle === 'straight') {
                            ...
                            this.drawStraightLine(startPoint, {
                                x: endX,
                                y: endY
                            });
                        } else {
                            // Kurva untuk temporary connection
                            this.drawCurvedLine(startPoint, {
                                x: endX,
                                y: endY
                            });
                        }
                    ...
                    exportMindmap() {
                        ...
                        this.nodes.forEach(node => {
                            ...
                            const props = ['fontSize', 'fontWeight', 'color', 'backgroundColor', 'backgroundImage',
                                'borderRadius', 'boxShadow', 'paddingTop', 'paddingBottom', 'paddingLeft',
                                'paddingRight', 'border', 'lineHeight', 'letterSpacing', 'textAlign'
                            ];
                            props.forEach(p => {
                                try {
                                    if (cs[p]) clone.style[p] = cs[p];
                                } catch (e) {
                                    /* ignore */
                                }
                            });
