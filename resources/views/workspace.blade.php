    ...
    @php
        ...
        if ($currentWorkspaceId) {
            ...
            if ($isCompanyAdmin) {
                ...
                $currentWorkspace = \App\Models\Workspace::where('company_id', $activeCompanyId)
                    ->active()
                    ->first();
            } else {
                // User biasa hanya bisa akses workspace yang mereka ikuti
                $currentWorkspace = $user->workspaces()->active()->first();
            }
    ...
    <div x-data x-init="$store.workspace = { selectedMenu: '' }" class="bg-[#f3f6fc] min-h-screen">
        ...
        <div class="p-8 grid grid-cols-3 gap-6 max-w-6xl mx-auto">
            ...
            @else
                <div class="bg-white rounded-2xl shadow-sm p-8 flex flex-col items-center justify-center opacity-50 cursor-not-allowed">
                    <div class="w-16 h-16 mb-4 text-gray-400">
                        <img src="{{ asset('images/icons/workspace_pengumuman.svg') }}" alt="Pengumuman Icon" class="w-full h-full">
                    </div>
                    <span class="text-gray-700 font-medium">Pengumuman</span>
                    <p class="text-xs text-gray-500 mt-2 text-center">Pilih workspace terlebih dahulu</p>
                </div>
            ...
            @else
                <div class="bg-white rounded-2xl shadow-sm p-8 flex flex-col items-center justify-center opacity-50 cursor-not-allowed">
                    <div class="w-16 h-16 mb-4 text-gray-400">
                        <img src="{{ asset('images/icons/workspace_dokumen&file.svg') }}" alt="Dokumen Icon" class="w-full h-full">
                    </div>
                    <span class="text-gray-700 font-medium">Dokumen</span>
                    <p class="text-xs text-gray-500 mt-2 text-center">Pilih workspace terlebih dahulu</p>
                </div>
                ...
                <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                    <div class="flex items-center">
                        ...
                        <p class="text-sm text-yellow-700">
                            ...
                            @else
                                Anda belum tergabung dalam workspace manapun atau belum memilih workspace. Silakan pilih workspace dari halaman <a href="{{ route('kelola-workspace') }}" class="underline font-medium">Kelola Workspace</a> terlebih dahulu.
                            @endif
                        </p>
                    </div>
                </div>
