...
<div x-data="briefReviewComponent()" class="max-w-6xl mx-auto px-6 py-10">
    ...
    <div class="mb-8 flex items-center justify-between">
        ...
        <a href="{{ route('brief.index') }}" class="text-sm font-semibold text-gray-500 hover:text-gray-700 flex items-center gap-1">
    ...
    <form action="{{ route('brief.approve') }}" method="POST" class="space-y-8">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            <div class="p-6 border-b border-gray-150 flex items-center justify-between">
                ...
                <div class="bg-blue-50 border border-blue-200 text-blue-700 font-medium text-xs px-3 py-1.5 rounded-full">
            ...
            <div class="p-6 bg-gray-50/50 grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="bg-white rounded-xl border border-gray-150 p-4 shadow-sm">
                    <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Tujuan Utama</label>
                    <input 
                        type="text" 
                        name="project_name" 
                        value="{{ $brief['project']['name'] ?? '' }}" 
                    ...
                    <textarea 
                        ...
                        placeholder="Deskripsi tujuan proyek..."
                    >{{ $brief['project']['goal'] ?? '' }}</textarea>
                </div>

                <!-- Deliverables -->
                <div class="bg-white rounded-xl border border-gray-150 p-4 shadow-sm">
                    <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Deliverables</label>
                    <textarea 
                        name="deliverables" 
                        rows="3"
                        class="w-full text-sm font-semibold text-gray-800 border-none p-0 focus:ring-0 resize-none" 
                        placeholder="Detail deliverable proyek..."
                    >{{ $brief['project']['deliverables'] ?? '' }}</textarea>
                </div>

                <!-- Deadline Utama -->
                <div class="bg-white rounded-xl border border-gray-150 p-4 shadow-sm flex flex-col justify-between">
                    <div>
                        <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Deadline Utama</label>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            <div class="p-6 border-b border-gray-150 flex items-center justify-between">
                ...
                <button type="button" @click="addTask()" class="text-[#225ad6] hover:text-[#1a44a6] text-sm font-bold flex items-center gap-1">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50/50">
                        <tr>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider w-5/12">Tugas</th>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider w-3/12">Pemilik</th>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider w-2/12">Priority</th>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider w-2/12">Tenggat</th>
                            <th scope="col" class="px-6 py-3 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider w-1/12">Aksi</th>
                        </tr>
                    </thead>
                        ...
                        <template x-for="(task, index) in tasks" :key="index">
                            <tr>
                                ...
                                <td class="px-6 py-4 text-center">
                                    <button type="button" @click="removeTask(index)" class="text-red-500 hover:text-red-700 p-1.5 rounded-lg hover:bg-red-50 transition">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="flex justify-end gap-3 pt-4">
            <a href="{{ route('brief.index') }}" class="bg-white hover:bg-gray-50 border border-gray-200 text-gray-700 font-medium text-sm px-6 py-3 rounded-lg transition">
                Batal
            </a>
            <button type="submit" class="bg-[#225ad6] hover:bg-[#1a44a6] text-white font-medium text-sm px-6 py-3 rounded-lg transition">
                Setujui & Buat Proyek
            </button>
        </div>
    </form>
