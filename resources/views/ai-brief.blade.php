                        ...
                        <div class="text-sm font-medium text-rose-500 flex items-center gap-2">
                            ...
                            <template x-if="isEditingDeadline">
                                <span class="flex items-center gap-1.5">
                                    <input type="text" x-model="mainDeadline" class="border border-slate-200 rounded px-2 py-0.5 text-xs focus:ring-1 focus:ring-blue-500 focus:outline-none text-slate-800 bg-white min-w-[120px]">
...
<script>
    function aiBriefComponent() {
        return {
            ...
            editDecisionForm: { title: '', sources: [] },
            availableFiles: @json($availableFiles),

            // ─── State: Ringkasan ───────────────────────────────────
            projectGoal: @json($brief['summary']['project_description'] ?? ''),
            openEditGoalModal: false,
            editGoalForm: { project_goal: '' },

            deliverables: @json($deliverablesLabel ?? ''),
            openEditDeliverablesModal: false,
            editDeliverablesForm: { deliverables: '' },

            // ─── State: Deadline ────────────────────────────────────
            mainDeadline: @json($brief['summary']['main_deadline'] ?? ''),
            isEditingDeadline: false,

            // ─── State: Task Modal ───────────────────────────────────
            openEditTaskModal: false,
            editingTaskIndex: null,
            editTaskForm: { title: '', description: '', priority: 'medium', deadline: '', assignee_id: '', _assignee_name: '' },

            // ─── Tasks CRUD ─────────────────────────────────────────
            addTask() {
                this.editingTaskIndex = null;
                this.editTaskForm = { title: '', description: '', priority: 'medium', deadline: '', assignee_id: '', _assignee_name: '' };
            ...
            addDecision() {
                ...
                this.editDecisionForm = { title: '', sources: [] };
